...
"이 사람이 지금 병원/약국을 찾는 중인가?" 를 판단해 팝업을 띄울지 정합니다.
...
ALLOWED_PLACE_TYPES = (PLACE_TYPE_HOSPITAL, PLACE_TYPE_PHARMACY)
...
class PlaceIntentRequest(BaseModel):
    ...
    # 1:1 방(2명)에서도 팝업은 떠야 하므로 v2 약속 추출과 달리 2명부터 받습니다.
...
class PlaceIntentDecision(BaseModel):
    ...
    place_type: Optional[str] = Field(
        default=None,
        description='찾는 장소 종류 ("HOSPITAL" / "PHARMACY"). SUPPRESS 면 null',
